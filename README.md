...
| [`interviews/`](interviews/) | Five system-design interview questions (healthcare RAG refactor, bank fraud copilot, insurer claims intake, marketplace multi-tenant support, pharma clinical search), each with no-infra / new-infra / FOSS-vs-Agent-Runtime parts and no answers; the `design-reviewer` agent in `.claude/agents/` scores your written answer against a hidden rubric; `past-problems/` (gitignored) holds your own past designs for the same reviewer. Off the study calendar |
